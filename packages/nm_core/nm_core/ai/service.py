...
def _citations(session: Session, user_id: uuid.UUID, ctx: ToolContext, text: str) -> list[Citation]:
    cited = dict(ctx.cited)
    # Also cite any case whose CNR appears verbatim in the answer (covers list answers).
    for case in CaseRepository(session).list_by_user(user_id):
        if case.cnr in text:
...
def ask(
    ...
    *,
    ...
    ctx = ToolContext(session, user.id)
    text = ""
    mode = "offline"
    if gemini.is_available():
        try:
            ...
            text = ""
    if not text:
        ctx = ToolContext(session, user.id)  # reset tool trace for the fallback run
